...
$pdo = ConnMySQL::makeConnect('localhost', 'root', '', 'blog');
$now = date('Y-m-d');
    ...
    switch ($subType) {
        case 0:
            ...
            for($i = 0; $i < count($artListAll); $i++) {
                ...
                $artList[$i]['time'] = $artListAll[$i]['arttime'];
                $artList[$i]['id'] = $artListAll[$i]['id'];
            }
            echo json_encode($artList);
            break;
        case 1:
            $result = $pdo->insert("article", "id, tag, content, arttime, title", "'', '$artTag', '$artContent', '$now', '$artTitle'");
            echo $result;
            break;
        case 2:
            $result = $pdo->updata("article", "tag='$artTag', content='$artContent', arttime='$now', title='$artTitle'", "id='$artId'");
            echo $result;
            break;
        case 3:
        ...
        // 通过时间查找
        case 6:
            $result = $pdo->query('article','arttime',$artTag);
            $artListAll = $result->fetchAll(PDO::FETCH_ASSOC);
            $artList = array();
            for($i = 0; $i < count($artListAll); $i++) {
                $artList[$i]['title'] = $artListAll[$i]['title'];
